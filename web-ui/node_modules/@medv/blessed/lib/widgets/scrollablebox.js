...
function ScrollableBox(options) {
  ...
  function createScrollSpeed(delay, fast, slow) {
    let prev = new Date();
    return () => {
      const now = new Date();
      const lines = now - prev < delay ? fast : slow;
      prev = now;
      return lines;
    };
  }

  let scrollSpeed = createScrollSpeed(20, 3, 1);
  if (this.screen.program.isHyper) {
    scrollSpeed = createScrollSpeed(10, 2, 1);
  }
  if (options.mouse) {

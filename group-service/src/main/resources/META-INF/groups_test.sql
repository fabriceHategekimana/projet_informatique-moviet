...
CREATE TABLE T_groups (group_id serial primary key, name varchar(255) not null, admin_id varchar(255) not null, group_status status_type DEFAULT 'CHOOSING', movie_winner_id int default -1);
...
CREATE TABLE T_users (user_id varchar(255) primary key);
...
CREATE TABLE T_groups_users (id serial primary key, group_id int REFERENCES T_groups(group_id), user_id varchar(255) REFERENCES T_users(user_id), user_status status_type NOT NULL DEFAULT 'CHOOSING', year_from int, year_to int);
...
CREATE TABLE T_groups_users_keywords (group_id int NOT NULL REFERENCES T_groups(group_id), user_id varchar(255) NOT NULL REFERENCES T_users(user_id), keyword_id int NOT NULL, primary key (group_id, user_id, keyword_id));
...
CREATE TABLE T_groups_users_genres (group_id int NOT NULL REFERENCES T_groups(group_id), user_id varchar(255) NOT NULL REFERENCES T_users(user_id), genre_id int NOT NULL, primary key (group_id, user_id, genre_id));
